...
export async function fetchProviderBalance(
  ...
  if (provider === 'sudo' && process.env.CARD_ISSUER_API_KEY) {
    try {
      const res = await fetch(
        `https://api.sudo.africa/accounts?number=${virtualAccountNumber}`,
        {
          headers: {
  ...
  if (provider === 'mono' && process.env.MONO_SECRET_KEY) {
    try {
      const res = await fetch(
        `https://api.withmono.com/v2/accounts?account_number=${virtualAccountNumber}`,
        {
          headers: {
      ...
      if (!res.ok) throw new Error(`Mono API ${res.status}`);
      const data: any = await res.json();
      const account = data?.data?.[0];
      return {
  ...
  if (provider === 'flutterwave' && process.env.FLW_SECRET_KEY) {
    try {
      const res = await fetch(
        `https://api.flutterwave.com/v3/virtual-account-numbers/${virtualAccountNumber}`,
        {
          headers: {
      ...
      if (!res.ok) throw new Error(`Flutterwave API ${res.status}`);
      const data: any = await res.json();
      return {

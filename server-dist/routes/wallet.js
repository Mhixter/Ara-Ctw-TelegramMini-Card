...
// ─────────────────────────────────────────────────────────────────────────────
router.get('/users/search', auth_1.requireAuth, auth_1.requireUUID, async (req, res) => {
    try {
        const q = String(req.query.q || '').trim().replace(/^@/, '').toLowerCase();
        if (!q || q.length < 2) {
            return res.status(400).json({ error: 'Search query too short (min 2 chars).' });
        }
        const result = await db_1.default.query(`SELECT id, first_name, username, kyc_status, is_active
         FROM users
...
// ─────────────────────────────────────────────────────────────────────────────
router.post('/send', auth_1.requireAuth, auth_1.requireUUID, async (req, res) => {
    ...
    try {
        ...
        const kycRes = await db_1.default.query('SELECT kyc_status FROM users WHERE id = $1', [senderUserId]);
        ...
        const walletsRes = await client.query(`SELECT w.*, u.first_name, u.username, u.telegram_id
         FROM wallets w
         JOIN users u ON u.id = w.user_id
        WHERE w.user_id = ANY($1::uuid[]) AND w.currency = 'NGN'
        ORDER BY w.user_id
        FOR UPDATE`, [walletIds]);
        const senderWallet = walletsRes.rows.find((w) => w.user_id === senderUserId);
        const recipientWallet = walletsRes.rows.find((w) => w.user_id === recipientUserId);
        if (!senderWallet) {
        ...
        const ledgerRes = await client.query(`SELECT
       ...
       AS ledger_balance
       FROM ledger_entries
       WHERE credit_wallet_id = $1 OR debit_wallet_id = $1`, [senderWallet.id]);
        ...
        const reference = `P2P-${(0, uuid_1.v4)()}`;
       ...
       VALUES ($1, $2, $3, $4, 'P2P_SEND', $5)`, [reference, senderWallet.id, recipientWallet.id, amount, meta]);
        // Update stored balances
        await client.query('UPDATE wallets SET balance = balance - $1, updated_at = NOW() WHERE id = $2', [amount, senderWallet.id]);
        await client.query('UPDATE wallets SET balance = balance + $1, updated_at = NOW() WHERE id = $2', [amount, recipientWallet.id]);
        ...
        if (recipientWallet.telegram_id) {
            ...
            (0, telegramNotify_1.sendTelegramMessage)(recipientWallet.telegram_id, [
                `💸 <b>Money Received!</b>`,
                ``,
                `💰 <b>Amount:</b> ₦${amtFmt}`,
        ...
        if (senderWallet.telegram_id) {
            ...
            (0, telegramNotify_1.sendTelegramMessage)(senderWallet.telegram_id, [
                `✅ <b>Transfer Sent</b>`,
                ``,
                `💰 <b>Amount:</b> ₦${amtFmt}`,
...
router.post('/fund', auth_1.requireAuth, auth_1.requireUUID, async (req, res) => {
    ...
    try {
       ...
       VALUES ($1, $2, $3, $4, $5)`, [reference, wallet.id, amount, 'WALLET_FUNDING', JSON.stringify({ source: 'manual', currency })]);
        await client.query('COMMIT');

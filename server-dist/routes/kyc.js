...
function hashField(value) {
    return crypto_1.default.createHash('sha256').update(value.trim() + ENCRYPTION_KEY).digest('hex');
}
...
router.get('/status', auth_1.requireAuth, auth_1.requireUUID, async (req, res) => {
    try {
        ...
        const kyc = await db_1.default.query(`SELECT full_name, date_of_birth, liveness_score, verified_at, country, id_type
       FROM user_kyc WHERE user_id = $1`, [req.user.userId]);
        res.json({
...
router.post('/tier1', auth_1.requireAuth, auth_1.requireUUID, async (req, res) => {
    ...
    try {
        const { fullName, dateOfBirth, country = 'NG', idType = 'BVN_NIN', idNumber, bvn, nin, } = req.body;
        ...
        if (existing.rows.length > 0) {
            await client.query(`UPDATE user_kyc
         SET bvn_hash=$1, nin_hash=$2, full_name=$3, date_of_birth=$4,
             country=$5, id_type=$6, id_number_hash=$7, updated_at=NOW()
         WHERE user_id=$8`, [bvnHash, ninHash, fullName.trim(), dateOfBirth, country, idType, idNumberHash, req.user.userId]);
        }
        else {
            await client.query(`INSERT INTO user_kyc
           (user_id, bvn_hash, nin_hash, full_name, date_of_birth, country, id_type, id_number_hash)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [req.user.userId, bvnHash, ninHash, fullName.trim(), dateOfBirth, country, idType, idNumberHash]);
        }
